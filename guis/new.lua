...
do
	...
	local function downloadFile(path, callback)
		if not isfile(path) then
			...
			local success, data = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/stepa444dss-wq/Vapev176Compiled/'..readfile('newvape/profiles/commit.txt')..'/'..select(1, path:gsub('newvape/', '')), true)
			end)
	...
	general:CreateButton({
		...
		vape.Save = function() end
			...
			if shared.VapeDeveloper then
				...
				loadstring(game:HttpGet('https://raw.githubusercontent.com/stepa444dss-wq/Vapev176Compiled/'..readfile('newvape/profiles/commit.txt')..'/loader.lua', true))()
			end
		end,
